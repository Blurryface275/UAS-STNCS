...
- [ ] **[Krusial - Implementasi Rute B]** Hubungkan aksi _Disetujui_ (Approve) bersamaan dengan menembak API _Payload_ lengkap tugas ini (termasuk `submitted_at`) menuju Hyperledger Fabric.
...
2. **Frontend (Transparansi Hash):** Cetak `$task['file_hash']` di bawah link file. Print juga `$task['latitude']`, `$task['longitude']`, serta `$task['submitted_at']` agar manajer yakin memeriksa data yang konkret.
3. **Backend (Approval Handler - Kunci Rute B):**
   ...
   - **Eksekusi Blockchain:** Tepat sedetik setelah meng-update `status = Disetujui` di MySQL, sistem _Backend_ mengunci paketan Karyawan (ID Penerima, Nama Pekerjaan, Hash File, Geolocation, dan Timestamp `submitted_at`) dan menembakkannya ke _Hyperledger Fabric API_! Bukti dibekukan permanen.
